...
function generateRoundRobinFixtures(teamIds, seasonId, startDate) {
    const fixtures = [];
...
export async function registerRoutes(app) {
    ...
    app.get("/api/venues", async (req, res) => {
        try {
            ...
            const venues = await storage.getVenues(verified === 'true', sportId);
    ...
    app.get("/api/venues/:id", async (req, res) => {
        try {
            ...
            return res.json(Object.assign(Object.assign({}, venue), { fields }));
        }
        catch (error) {
            return res.status(500).json({ error: error.message });
        }
    ...
    app.get("/api/slots/search", async (req, res) => {
        try {
            ...
            const slots = await storage.searchAvailableSlots(fieldId, new Date(startTime), new Date(endTime));
    ...
    app.post("/api/bookings/:bookingId/cancel", authenticate, async (req, res) => {
        try {
            ...
            if (payment && payment.status === "succeeded") {
                ...
                await notificationService.sendToUser(userId, notifications.refundIssued(payment.amountPkr));
            }
    ...
    app.post("/api/payments/webhook/:provider", async (req, res) => {
        ...
        if (provider === "stripe") {
            ...
            // For Firebase Functions, req.rawBody is often available. For Express, need body-parser.raw()
            if (req.rawBody) {
                rawBody = req.rawBody;
            }
            else {
                ...
                console.warn("req.rawBody not found. Webhook signature verification might fail.");
            ...
            if (provider === "stripe" && event) {
                switch (event.type) {
                    case 'payment_intent.succeeded':
                        ...
                        const bookingIdSucceeded = (_a = paymentIntentSucceeded.metadata) === null || _a === void 0 ? void 0 : _a.bookingId;
                        const userIdSucceeded = (_b = paymentIntentSucceeded.metadata) === null || _b === void 0 ? void 0 : _b.userId;
                ...
                if (status === 'succeeded') {
                    ...
                    if (payment === null || payment === void 0 ? void 0 : payment.bookingId) {
    ...
    app.post("/api/games", authenticate, async (req, res) => {
        try {
            ...
            const game = await storage.createGame(Object.assign(Object.assign({}, data), { sportId: data.sportId }));
    ...
    app.get("/api/games/search", authenticate, async (req, res) => {
        try {
            ...
            const userId = req.user.uid;
            const games = await storage.getGames(userId, sportId === 'all' ? undefined : sportId, skillLevel);
    ...
    app.post("/api/wallet/add-funds/intent", authenticate, async (req, res) => {
        try {
            const { amountPkr, provider = "mock", idempotencyKey } = req.body;
            const userId = req.user.uid;
            if (!idempotencyKey) {
                return res.status(400).json({ error: "idempotencyKey is required" });
            }
            ...
            const walletPayment = await storage.createWalletPayment({
                ...
                idempotencyKey,
                status: "pending",
                redirectUrl: `${req.protocol}://${req.get('host')}/dashboard/wallet` // Redirect to wallet page
            });
    ...
    app.post("/api/games/:gameId/cancel", authenticate, async (req, res) => {
        try {
            ...
            if (waitlist.length > 0) {
                ...
                await notificationService.sendToUser(firstInWaitlist.userId, notifications.waitlistSpotOpen(gameId, joinToken));
        ...
        try {
            if (provider === "stripe" && event) {
                switch (event.type) {
                    case 'payment_intent.succeeded':
                        const paymentIntentSucceeded = event.data.object;
                        const gameIdSucceeded = (_a = paymentIntentSucceeded.metadata) === null || _a === void 0 ? void 0 : _a.gameId;
                        const userIdSucceeded = (_b = paymentIntentSucceeded.metadata) === null || _b === void 0 ? void 0 : _b.userId;
                            ...
                            if (gamePayment) {
                                ...
                                if (game) {
                                    ...
                                    if (wasFilled) {
                                        ...
                                        await notificationService.sendToMultipleUsers(playerIds, notifications.gameFull(game.sport.name));
                                    }
                                    else if (game.currentPlayers >= game.minPlayers && game.status === 'open') {
                                        await storage.updateGameStatus(game.id, 'confirmed');
                                    }
                                    // Notify the joining player
                                    await notificationService.sendToUser(userIdSucceeded, notifications.paymentSuccess(gamePayment.amountPkr));
                                    // Notify host about new player
                                    if (!wasFilled) {
                                        await notificationService.sendToUser(game.hostId, notifications.gameJoined(game.sport.name, game.currentPlayers, game.maxPlayers));
                                    }
                                }
                            }
                        }
                        break;
                    case 'payment_intent.payment_failed':
                        const gamePaymentIntentFailed = event.data.object;
                ...
                if (status === 'succeeded') {
                    ...
                    if (payment) {
                        ...
                        if (game) {
                            ...
                            // Notify the joining player
                            await notificationService.sendToUser(payment.userId, notifications.paymentSuccess(payment.amountPkr));
                            // Notify host about new player
                            if (!wasFilled) {
                                await notificationService.sendToUser(game.hostId, notifications.gameJoined(game.sport.name, game.currentPlayers, game.maxPlayers));
                            }
        ...
        try {
            if (provider === "stripe" && event) {
                switch (event.type) {
                    case 'payment_intent.succeeded':
                        const paymentIntentSucceeded = event.data.object;
                        const userIdSucceeded = (_a = paymentIntentSucceeded.metadata) === null || _a === void 0 ? void 0 : _a.userId;
                        const typeSucceeded = (_b = paymentIntentSucceeded.metadata) === null || _b === void 0 ? void 0 : _b.type;
                        if (userIdSucceeded && typeSucceeded === "wallet_topup") {
                            ...
                            if (walletPayment) {
                                ...
                                await notificationService.sendToUser(userIdSucceeded, notifications.walletTopUpSuccess(walletPayment.amountPkr));
                            }
                        }
                        break;
                    case 'payment_intent.payment_failed':
                ...
                if (status === 'succeeded') {
                    ...
                    if (payment) {
                        ...
                        await notificationService.sendToUser(payment.userId, notifications.walletTopUpSuccess(payment.amountPkr));
                    }
                }
            }
            return res.json({ received: true });
    ...
    app.post("/api/wallet/withdraw-funds", authenticate, async (req, res) => {
        try {
    ...
    app.post("/api/leagues/season", authenticate, async (req, res) => {
        try {
            ...
            const season = await storage.createSeason(Object.assign(Object.assign({}, data), { sportId: data.sportId }));
    ...
    app.post("/api/leagues/season/:id/team", authenticate, async (req, res) => {
        try {
            ...
            const data = insertTeamSchema.parse(Object.assign(Object.assign({}, req.body), { seasonId }));
