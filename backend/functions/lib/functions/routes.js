...
export async function registerRoutes(app) {
    ...
    app.get("/api/games/search", authenticate, async (req, res) => {
        try {
            const userId = req.user.uid;
            const games = await storage.getGames(userId, req.query.sportId === 'all' ? undefined : req.query.sportId, req.query.skillLevel);
    ...
    app.post("/api/game-pay/:gameId/intent", authenticate, async (req, res) => {
        try {
            const { gameId } = req.params;
            const { gameId, userId, provider = "mock", idempotencyKey } = req.body;
            if (!idempotencyKey) {
                return res.status(400).json({ error: "idempotencyKey is required" });
            }
