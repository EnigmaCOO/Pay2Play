...
import type {
  ...
  WalletPayment, InsertWalletPayment,
  Refund, InsertRefund,
  Season, InsertSeason,
...
export interface IStorage {
  ...
  // Wallet Payments
  createWalletPayment(payment: InsertWalletPayment): Promise<WalletPayment>;
  getWalletPayment(id: string): Promise<WalletPayment | undefined>;
  getWalletPaymentByIdempotencyKey(key: string): Promise<WalletPayment | undefined>;
  updateWalletPaymentStatus(id: string, status: string, providerRef?: string): Promise<void>;
  ...
  createStripeRefund(paymentId: string, amount: number): Promise<Refund>;
...
export class DbStorage implements IStorage {
    ...
    if (payment.provider === "stripe") {
      try {
        ...
        const [created] = await db.insert(schema.payments).values({
          ...payment,
          providerRef: paymentIntent.client_secret, // Store client_secret here
          status: "pending", // Initial status for Stripe Payment Intent
        }).returning();
  ...
  async createGamePayment(payment: InsertGamePayment): Promise<GamePayment> {
    const db = this.ensureDb();
    let createdGamePayment: GamePayment;
    ...
    if (payment.provider === "stripe") {
      try {
        ...
        const [created] = await db.insert(schema.gamePayments).values({
          ...payment,
          providerRef: paymentIntent.client_secret, // Store client_secret here
          status: "pending", // Initial status for Stripe Payment Intent
        }).returning();
  ...
  // Wallet Payments
  async createWalletPayment(payment: InsertWalletPayment): Promise<WalletPayment> {
    const db = this.ensureDb();
    let createdWalletPayment: WalletPayment;
    ...
    if (payment.provider === "stripe") {
      try {
        ...
        const [created] = await db.insert(schema.walletPayments).values({
          ...payment,
          providerRef: paymentIntent.client_secret, // Store client_secret here
          status: "pending", // Initial status for Stripe Payment Intent
        }).returning();
  ...
  async getWalletPayment(id: string): Promise<WalletPayment | undefined> {
    const db = this.ensureDb();
    const [payment] = await db.select().from(schema.walletPayments).where(eq(schema.walletPayments.id, id));
    return payment;
  }

  async getWalletPaymentByIdempotencyKey(key: string): Promise<WalletPayment | undefined> {
    const db = this.ensureDb();
    const [payment] = await db.select().from(schema.walletPayments).where(eq(schema.walletPayments.idempotencyKey, key));
    return payment;
  }

  async updateWalletPaymentStatus(id: string, status: string, providerRef?: string): Promise<void> {
    const db = this.ensureDb();
    const updates: any = { status: status as any, updatedAt: new Date() };
    if (providerRef) updates.providerRef = providerRef;
  ...
  async createStripeRefund(paymentId: string, amount: number): Promise<Refund> {
    const db = this.ensureDb();
...
class InMemoryStorage implements IStorage {
  ...
  private walletPayments: WalletPayment[] = [];
  ...
  async updateUserWalletBalance(userId: string, amount: number): Promise<void> {
    const user = this.users.find((item) => item.id === userId);
    if (user) {
      user.balancePkr = (user.balancePkr ?? 0) + amount;
    }
  }
  ...
  async updateWalletPaymentStatus(id: string, status: string, providerRef?: string): Promise<void> {
    const payment = this.walletPayments.find((item) => item.id === id);
    if (payment) {
      payment.status = status as WalletPayment["status"];
      payment.updatedAt = new Date();
      if (providerRef) {
        payment.providerRef = providerRef;
      }
