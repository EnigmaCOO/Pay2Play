...
import type { InsertVenue, InsertField, InsertSlot, InsertGame, InsertTeam } from "./shared/schema.js";
...
export async function seed() {
  ...
  try {
    ...
    const getSportId = (name: string) => allSports.find(s => s.name === name)?.id || "";
    ...
    const fields: InsertField[] = [
      { venueId: createdVenues[0].id, name: "Cricket Ground A", sportId: getSportId("cricket"), pricePerHourPkr: 3000, capacity: 22 },
      { venueId: createdVenues[0].id, name: "Football Field 1", sportId: getSportId("football"), pricePerHourPkr: 2500, capacity: 22 },
      { venueId: createdVenues[1].id, name: "Futsal Court 1", sportId: getSportId("futsal"), pricePerHourPkr: 2000, capacity: 10 },
