...
### Concept Snapshot
**Mock images:** `./screens/IMG_3452.jpeg`, `./screens/IMG_3459.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3451.jpeg`, `./screens/IMG_3461.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3453.jpeg`, `./screens/IMG_3456.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3442.jpeg`, `./screens/IMG_3447.jpeg`, `./screens/IMG_3448.jpeg`, `./screens/IMG_3454.jpeg`, `./screens/IMG_3455.jpeg`, `./screens/IMG_3464.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3448.jpeg`, `./screens/IMG_3450.jpeg`, `./screens/IMG_3463.jpeg`, `./screens/IMG_3465.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3463.jpeg`, `./screens/IMG_3465.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3444.jpeg`, `./screens/IMG_3446.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3443.jpeg`, `./screens/IMG_3445.jpeg`, `./screens/IMG_3449.jpeg`, `./screens/IMG_3453.jpeg`, `./screens/IMG_3456.jpeg`, `./screens/IMG_3457.jpeg`, `./screens/IMG_3462.jpeg`, `./screens/IMG_3466.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3460.jpeg`  
...
### Concept Snapshot
**Mock images:** `./screens/IMG_3458.jpeg`  
...
## Addendum – Executed FRD v0.1 (2025-12-11)

# Pay2Play – Players App FRD

Version: 0.1 (2025-12-11)
Status: Draft
Owners: Product (Players App), Engineering (Players App)
Platforms: iOS & Android (React Native + Expo)

## Purpose & Scope
- Define functional requirements for the player-facing Pay2Play app that lets users discover venues, see real-time slots, book, pay, and join host-led games.
- Covers onboarding, discovery, booking, payments, messaging, loyalty, notifications, and support experiences for players.
- Out of scope for this FRD: venue/admin dashboards, backoffice ops tooling, tournament/league management, and advanced social feed (future).

## References
- PRD: `shared/PLAYERS_APP_PRD.md`
- Wireframes: `shared/PLAYERS_APP_WIREFRAMES.md`
- Venue docs: `shared/VENUES_APP_FRD.md`, `shared/VENUES_APP_WIREFRAMES.md`
- Payments: JazzCash, Easypaisa, Stripe docs
- Architecture: shared design system & domain models in `shared/`

## Goals & Success Metrics (Player App)
- Book a slot from cold start in ≤60 seconds on median network.
- 99% OTP success rate within 2 attempts; <15s to verified session.
- Venue detail → booking conversion rate uplift vs baseline (track CTR to booking CTA and completion).
- Reduce booking failures due to availability conflicts to <0.5% of attempts.
- NPS/CSAT for booking flow ≥4.5/5 in beta.

## Personas & Roles
- Player (guest, new, returning): browses, books, pays, joins matches.
- Host player: creates a match, sets rules, invites players, may collect per-seat payments.
- Admin (backoffice): manages venues/slots and resolves payments; not exposed in the player app UI but appears in assumptions.

## Assumptions & Dependencies
- Firebase Auth (phone OTP, Google, Apple) is available in target regions.
- Payments: JazzCash/Easypaisa for PK, Stripe for cards (test + live keys). PSP webviews are allowed inside Expo.
- Firestore holds authoritative slot/booking state; functions perform atomic reservation writes.
- Push notifications via FCM/APNs; SMS only for OTP.
- Device permissions (location, notifications) requested contextually with fallbacks.
...
- Show welcome screen with brand/value props and primary CTA (`Get Started`). Secondary guest explore mode shows limited discovery (no booking/payment).
- Phone login with OTP (primary):
  - Capture country code + number, validate format, and rate-limit OTP requests (e.g., 5 attempts/hour, 30s resend gate).
  - OTP verification creates Firebase Auth user; handle error states (expired/invalid/too many attempts).
- Social login (Google, Apple) optional but should create/link to same user profile; if phone exists, prompt to link.
- Minimal required profile after auth: name, WhatsApp-ready phone, preferred sport, city. Ask for location permission with clear rationale.
- Session persistence: silent re-auth on app open; explicit logout clears tokens, cached PII, and notification topics.
- Failed/edge flows: offline warning, retry, and clear messaging for blocked/unsupported numbers.

### 2) Profile & Account
- View/edit profile: name, photo, preferred sports, skill level, city/area, gender (optional), birthday (optional), communication preference.
- Manage contact methods: primary phone (read-only if tied to auth), secondary phone/email for comms, notification toggles (booking, matches, marketing opt-in with consent text).
- View past and upcoming bookings with status (confirmed, pending payment, cancelled, refunded) and tap into booking detail.
- Saved payment methods: show last-used PSP token when supported; allow removal. Do not store raw card data.
- Receipts/Invoices: downloadable/forwardable from booking detail (PDF or HTML view).

### 3) Venue Discovery & Search
- Home feed: nearby venues sorted by relevance (distance, rating, discounts, available slots soon). Requires coarse location or selected city.
- Search: free-text query (venue, sport, area) with debounced suggestions.
- Filters: sport, date, start time range, price range, distance, amenities (lights, parking, locker, indoor/outdoor, turf type), rating, discount flag, surface type, gender-friendly/women-only slots (where applicable).
- Sorting: closest, lowest price, highest rating, soonest available slot, best discount.
- Views: list and map toggle. Map pins show top 1–2 price/discount badges and tap-through to detail.
- Real-time slot indicators on cards for selected date (next available time & from-price).

### 4) Real-time Availability & Booking
- Venue detail: photos, address/map, facilities, rules, cancellation policy, pricing by sport/field/duration, discounts, and reviews summary.
- Slots: selectable by date; show duration (e.g., 60/90 mins), start time, field, price, and discount badge. Disable unavailable slots.
- Concurrency control: selecting a slot creates a short hold (e.g., 5 minutes) in Firestore/Functions; expired holds release automatically.
- Booking flow:
  - Select slot → confirm field/duration → number of players (for cost split calculations) → add-ons (equipment, water, coach) if offered → promo code entry.
  - Price breakdown: base, add-ons, discounts, service fee, tax, total. Always show currency.
  - Payments required before confirmation unless venue allows pay-on-arrival (flag per venue).
- Booking statuses: `draft` (held), `pending_payment`, `confirmed`, `cancelled_user`, `cancelled_admin`, `expired_hold`, `refunded`.
- Cancellation policy per venue: show rules before pay; enforce refund eligibility logic client-side and server-side.
- Reschedule: if venue supports, allow single reschedule within policy; otherwise require cancel + rebook.
- Confirmation: show success screen with QR/booking code, venue contact, “Invite players” CTA, and add-to-calendar.
- Failure handling: release hold, surface reason, allow retry.

### 5) Payments & Receipts
- PSP options: JazzCash, Easypaisa, Stripe (card/Apple Pay/Google Pay where available). Show dynamically based on country/device.
- Payment initiation from booking detail with redirect/webview or native sheet (Stripe). Display spinner + “Do not close” messaging.
- Payment outcomes: success → confirm booking; failure/timeout → return to booking detail with retry and reason code. Partial authorizations are treated as failure unless PSP guarantees completion.
- Refunds: trigger via support/admin; surface refunded amount and method to user; status updates via webhook → booking state.
- Receipts: store PSP reference, amount, currency, tax, discounts, payer phone/email. Available in booking detail and export/share.

### 6) Host-led Matches & Player Matching
- Host match creation:
  - Input: sport, venue/field, date/time (uses same slot inventory), match size (#players), skill level band, gender rules, visibility (public/link-only), per-seat price or free, payment requirement (pay to reserve vs pay-at-venue), description, rules.
  - Host can optionally pre-book the slot as part of creation; otherwise app blocks creation if slot unavailable.
- Match listing:
  - Discoverable via “Matches” tab and venue detail; filter by sport, date, location, skill, price, seats open.
  - Card shows seats left, price, start time, host rating (if available).
- Joining a match:
  - Seat reservation requires payment if host set pay-to-join; otherwise free hold until start.
  - Prevent overbooking; waitlist when full with notifications on open seats.
- Host controls: approve/auto-approve joiners, kick/ban, share invite link/deeplink, close match, cancel (with refund rules mirrored to booking).
- Match statuses: `draft`, `open`, `full`, `in_progress`, `completed`, `cancelled`.

### 7) In-App Messaging
- Contextual threads per booking and per match; group chat includes host + participants.
- Capabilities: send text, emojis, system messages (booking confirmed/cancelled, payment failed, player joined/left). Image/GIF not required for MVP.
- Read receipts (basic) and mute thread per user.
- Moderation: block/report user, remove participant (host), profanity filter on client if possible.

### 8) Loyalty & Rewards
- Points accrual per completed booking (configurable multiplier per currency/sport) and bonus for host creating full matches.
- Reward balances visible in profile and at checkout; redemption toggle if enabled for venue.
- Promo codes: percentage or fixed; validate eligibility (venue, sport, min spend, expiry, per-user limit).
- Badges/tiers (e.g., Rookie/Pro) displayed in profile and match cards if earned.

### 9) Notifications & Communications
- Push notifications for: OTP fallback (if allowed), booking confirmation/changes, payment success/fail, upcoming booking reminders (24h + 2h), match join/approval, chat messages (per thread subscription), rewards earned.
- In-app banners for critical events when app is foregrounded.
- Email/SMS (where permitted) for receipts and critical booking state changes.
- User-level notification preferences respected (marketing optional, transactional mandatory with clear labeling).

### 10) Settings, Help & Support
- Manage notification toggles, language (if/when Urdu added), measurement units (km/mi if applicable), theme (follow system).
- Help center links (FAQ), contact support (email/WhatsApp deep link), and share app link.
- Legal screens: Terms, Privacy, Refund/Cancellation policies; accessible from onboarding and settings.

### 11) Observability & Analytics
- Client events: app_open, onboarding_start, otp_request, otp_verify_success/fail, search_performed, filter_applied, venue_view, slot_selected, booking_started, payment_initiated, payment_success/fail, booking_confirmed, booking_cancelled, match_created, match_joined, chat_sent, reward_redeemed.
- Attach user/session, device, city, sport, venue_id, booking_id/match_id where relevant.
- Crash/ANR monitoring enabled; log client errors with redaction of PII.

### 12) Offline & Error Handling
- Inform user when offline; allow browsing cached venues/slots with clear “data may be stale” label; block booking/payment while offline.
- Graceful timeouts with retry and PSP-specific guidance; never charge twice on retries.

---

## Non-Functional Requirements
- Performance: key screens (home, venue detail, booking confirmation) load usable content <3s on 3G/low-mid devices; API p50 <400ms, p95 <900ms for read endpoints.
- Availability: 99.5% monthly for booking and payment paths; graceful degradation for non-critical features (chat, rewards) if needed.
- Scalability: support spike of 50 concurrent bookings/minute per city with no double-booking; read paths support 200 RPS across regions.
- Security & Privacy: all traffic over TLS; PII encrypted at rest (Firestore + Storage defaults); PSP tokens only; adhere to platform privacy (IDFA consent when used); store minimal PII on device.
- Accessibility: WCAG AA contrast; dynamic type/resizable text; voice-over labels; focus order correct.
- Localization: English primary; framework ready for Urdu with RTL support.
- Compliance: follow PSP KYC limits; consent for marketing communications; data retention for receipts per local law.

---

## User Stories (selected)
- As a new user, I can sign up with my phone and verify with OTP in <2 attempts so I can proceed to booking.
- As a guest, I can browse venues and see prices/slots but am prompted to sign in before checkout.
- As a player, I can filter venues by sport, time, and price to find a slot that fits my schedule.
- As a player, I can book a slot, pay securely, and receive a receipt and calendar invite.
- As a player, I can cancel within the venue policy and see whether I get a refund.
- As a host, I can create a match, set seat price, and approve joiners to fill the game.
- As a player, I can join a public match, pay my seat fee, and see the group chat.
- As a player, I receive reminders and directions so I arrive on time.
- As a player, I earn rewards after completing bookings and can view my balance.

Acceptance for each story: flow completes without crashes; errors are actionable; state is persisted (booking/match visible in profile), and notifications are delivered when applicable.

---

## Architecture & Technical Specification
- Frontend: React Native + Expo (managed workflow), TypeScript, shared UI kit from `shared/ui`, theming with dark-first palette.
- Backend: Firebase Functions (TypeScript), Firestore (normalized collections for users, venues, fields, slots, bookings, matches, payments, rewards), Firebase Storage for images.
- Realtime: Firestore listeners for chat and booking/match status; FCM for push.
- Authentication: Firebase Auth (phone OTP, Google, Apple). Account linking enforced to avoid duplicates.
- Payments: server-side intents/tokens per PSP; client receives redirect URLs or ephemeral keys (Stripe) and handles webview/sheet lifecycle.
- Feature flags/config: remote config per env (dev/stg/prod) controlling payment providers, discounts, rewards, guest mode, chat availability.
- Deep links: support venue detail, booking detail, match invite, and app-store fallback.
- Analytics: Firebase/Segment events with minimal PII.

---

## Acceptance Criteria (key flows)
- Onboarding: OTP request throttled; verification success creates user; retry shows error messages; guest mode blocked at payment.
- Discovery: Filters and sort persist during session; map/list toggles without losing state; selecting a venue shows accurate pricing for selected date.
- Booking: Slot selection creates hold; payment success confirms booking and releases hold; payment failure keeps slot available; confirmation screen shows booking code and share options.
- Payments: Supported providers appear per region/device; PSP failures surface reason; duplicate charges prevented on retries; receipts stored and viewable.
- Matches: Host cannot open match on unavailable slot; joining respects capacity; waitlist triggers notification; host actions (approve/kick/cancel) propagate to members.
- Messaging: Chat loads within 3s; sending message works online; receiving system messages on status changes.
- Rewards: Points accrue after booking completion; redemption toggle only when balance and eligibility allow; promo validation blocks ineligible codes.

---

## KPIs & Measurement
- Time-to-book (app open → booking confirmed).
- OTP success rate and resend count.
- Venue detail → booking initiation conversion; booking initiation → payment success rate.
- Booking failure reasons (availability conflict, payment fail).
- Match fill rate and time-to-fill.
- Rewards redemption rate and repeat bookings per cohort.

---

## User Flows (textual)
- Onboarding: Open app → Welcome → Get Started → Phone entry → OTP verify → Minimal profile → Home feed.
- Guest browse: Open app → Explore first → Discovery with limited CTAs → Prompt to sign in before slot hold.
- Booking: Home/Search → Venue detail → Select date/slot → Price breakdown → Pay → Confirmation → Calendar + Share.
- Host match: Create match → Pick venue/slot → Set seats/price/rules → Publish → Invite → Players join/pay → Host manages → Match completes.
- Join match: Discover match → View details → Pay to join (if required) → Added to roster/chat → Reminders → Attendance.

---

## UI Mockups
- Source of truth: `shared/PLAYERS_APP_WIREFRAMES.md` and linked Figma mockups. FRD defers to PRD/wireframes for visual details; FRD focuses on behavior and data.

---

## API Endpoints (contract-level)
- `POST /api/auth/otp/start` – start OTP for phone; rate-limit metadata returned.
- `POST /api/auth/otp/verify` – verify code and return auth tokens/user record.
- `GET /api/users/{id}` – fetch profile, preferences, rewards summary, notification settings.
- `PATCH /api/users/{id}` – update profile/preferences.
- `GET /api/venues` – list venues with filters/sort/pagination; supports location params.
- `GET /api/venues/{id}` – venue detail with fields, amenities, policies.
- `GET /api/venues/{id}/slots?date=YYYY-MM-DD` – real-time slots for a venue/field/date.
- `POST /api/bookings` – create booking hold with selected slot/add-ons and pricing snapshot.
- `POST /api/bookings/{id}/confirm` – finalize booking and start payment intent.
- `GET /api/bookings/{id}` – booking detail including payment status and receipt references.
- `POST /api/bookings/{id}/cancel` – cancel with reason; returns refund eligibility/outcome.
- `POST /api/payments/{bookingId}/provider/{provider}` – initiate payment with provider-specific payload (Stripe intent, JazzCash/Easypaisa redirect data).
- `POST /api/matches` – create host-led match bound to a slot.
- `GET /api/matches` – list matches with filters.
- `GET /api/matches/{id}` – match detail including roster and chat thread id.
- `POST /api/matches/{id}/join` – join/pay for a seat; handles waitlist when full.
- `POST /api/matches/{id}/close` – host closes/cancels match.
- `POST /api/chat/{threadId}/messages` – send chat message; Firestore realtime used for reads.
- Webhooks (server-initiated): PSP payment updates; booking state sync.

---

## Data Models (high level)
- User: id, phone, email (optional), name, photoUrl, city, preferredSports[], skillLevel, notificationPrefs, rewardBalance, createdAt, lastActiveAt, authProviders[].
- Venue: id, name, address, geo, city, photos[], amenities[], policies (cancellation/reschedule), contact, rating, discountRules, supportedPayments, sports[].
- Field: id, venueId, name, sport, surface, indoor, lights, size, priceRules (by duration/time/day), addOns[].
- Slot: id, venueId, fieldId, date, startTime, duration, price, discount, holdExpiresAt, status (available/held/booked/blocked), source (venue/admin/system).
- Booking: id, userId, venueId, fieldId, slotId, status, participantsCount, priceBreakdown, promoCode, paymentProvider, paymentStatus, receiptUrl, cancellationPolicySnapshot, calendarInfo, createdAt, updatedAt.
- Match: id, hostUserId, bookingId/slotRef, sport, skillLevel, seatsTotal, seatsFilled, pricePerSeat, visibility, rules, status, participants[], waitlist[], chatThreadId, createdAt.
- Payment: id, bookingId, provider, amount, currency, status (initiated/pending/succeeded/failed/refunded), providerRef, refundInfo, createdAt, updatedAt.
- Reward: userId, balance, accrualEvents[], redemptionEvents[]; promo codes with constraints (sport/venue/min spend/expiry/max uses).

---

## Security Considerations
- Authentication/authorization for all API calls; enforce ownership on bookings/matches; admin-only mutations guarded by role claims.
- Firestore security rules aligned to domain models; read/write limited to user-owned docs and public discovery collections.
- PSP: no card data stored; use tokens/intents; webhook secrets validated; idempotent payment handlers.
- Data protection: PII minimized; cache encryption where feasible; secure storage for refresh tokens.
- Audit trails: keep mutation logs for bookings, payments, and match membership changes.

---

## Deployment Plan
- CI/CD via GitHub Actions (lint, tests, typecheck, build). Preview builds to Expo EAS for testers; main → staging; tagged release → production.
- Firebase Functions deployed per environment (dev/stg/prod) with separate projects/configs.
- Feature flags to roll out payments, chat, and rewards gradually; kill-switch for PSP outages.
- Crash/analytics dashboards monitored; on-call alerts for payment failure spikes and booking conflict rate.

---

## Testing Strategy
- Unit tests: booking price calc, promo validation, hold/expiry logic, reward accrual.
- Integration tests: booking + payment intent per provider; webhook processing; match join/capacity enforcement; Firestore rules tests.
- Component tests: key screens (onboarding, search, venue detail, checkout) using RN Testing Library.
- End-to-end: onboarding → booking → payment → receipt; host creates match → player joins; cancellation/refund paths.
- Performance tests: slot listing under load; cold start metrics on mid-tier Android.
- Manual QA: payment edge cases (timeouts/back), network loss during pay, locale changes, accessibility checks.

---

## Future Enhancements
- Split payments between players; pay-later/BNPL where allowed.
- Advanced matchmaking (skill/ELO, availability calendar syncing, invites with deposit logic).
- Tournaments/leagues and team profiles.
- Social feed/highlights; richer media in chat.
- Subscriptions or membership passes; gift cards.
